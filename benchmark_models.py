...
def train(precision, device):
    ...
    for model_type in MODEL_LIST.keys():
        for model_name in MODEL_LIST[model_type]:
            ...
            print(
                f'Training {precision} precision {model_name} model average train time : {sum(durations) / len(durations)}ms')
            del model
            ...
            for model_name in MODEL_LIST[model_type]:
                ...
                for step, img in enumerate(rand_loader):
                    ...
                    if step >= args.WARM_UP:
                        durations.append((end - start) * 1000)
                print(
                    f'Inference {precision} precision {model_name} model average inference time : {sum(durations) / len(durations)}ms')
                del model
                benchmark[model_name] = durations
    return benchmark
